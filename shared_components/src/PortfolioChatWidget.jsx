...
function SparkIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className="h-5 w-5 fill-current">
      <path d="M12 3.25c-4.83 0-8.75 3.48-8.75 7.78 0 2.5 1.31 4.71 3.34 6.14v3.08l3.04-1.66c.72.15 1.46.22 2.23.22 4.83 0 8.75-3.48 8.75-7.78S16.83 3.25 12 3.25Zm-2.92 6.84a1.1 1.1 0 1 1 0 2.2 1.1 1.1 0 0 1 0-2.2Zm2.92 0a1.1 1.1 0 1 1 0 2.2 1.1 1.1 0 0 1 0-2.2Zm2.92 0a1.1 1.1 0 1 1 0 2.2 1.1 1.1 0 0 1 0-2.2Z" />
    </svg>
  );
}
...
function CloseIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className="h-5 w-5 fill-current">
      <path d="m18.3 6.76-1.06-1.06L12 10.94 6.76 5.7 5.7 6.76 10.94 12 5.7 17.24l1.06 1.06L12 13.06l5.24 5.24 1.06-1.06L13.06 12l5.24-5.24Z" />
    </svg>
  );
}
...
function SendIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className="h-4 w-4 fill-current">
      <path d="M3.35 20.65 21 12 3.35 3.35 3.3 10.1l12.2 1.9-12.2 1.9.05 6.75Z" />
    </svg>
  );
}
...
function BotBubbleIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className="h-4 w-4 fill-current">
      <path d="M12 3.5c-4.97 0-9 3.58-9 8 0 2.57 1.37 4.85 3.5 6.31V21l3.17-1.73c.75.16 1.53.23 2.33.23 4.97 0 9-3.58 9-8s-4.03-8-9-8Zm-3 6.5a1.25 1.25 0 1 1 0 2.5A1.25 1.25 0 0 1 9 10Zm3 0a1.25 1.25 0 1 1 0 2.5 1.25 1.25 0 0 1 0-2.5Zm3 0a1.25 1.25 0 1 1 0 2.5 1.25 1.25 0 0 1 0-2.5Z" />
    </svg>
  );
}
...
function PortfolioChatWidget({
  ...
  async function sendMessage(rawMessage) {
    ...
    try {
      ...
      const answer = response.ok
        ? data.answer
        : data.error || 'The portfolio assistant is unavailable right now.';

      setMessages((prev) => [
        ...prev,
        { id: `assistant-${Date.now()}`, role: 'assistant', text: answer || 'I only answer questions about this portfolio.' }
      ]);
    } catch (error) {
      setMessages((prev) => [
        ...prev,
        { id: `assistant-error-${Date.now()}`, role: 'assistant', text: 'The portfolio assistant is unavailable right now.' }
      ]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end gap-3 md:bottom-8 md:right-8">
      {isOpen && (
        <section className="w-[calc(100vw-2rem)] max-w-sm overflow-hidden rounded-[1.75rem] border border-slate-800 bg-slate-950/95 shadow-[0_20px_70px_rgba(2,6,23,0.55)] backdrop-blur overscroll-contain">
          <div className="border-b border-slate-800 px-5 py-4">
            <div className="flex items-center gap-3">
              <span className="inline-flex h-10 w-10 items-center justify-center rounded-2xl bg-cyan-300/12 text-cyan-300">
                <BotBubbleIcon />
              </span>
              <div className="min-w-0">
                <h2 className="font-sans text-base font-bold text-slate-100">{title}</h2>
          ...
          <div ref={messagesRef} className="max-h-[26rem] space-y-3 overflow-y-auto overscroll-contain px-5 py-4">
            ...
            {messages.map((message) => (
              <div
                ...
                className={`max-w-[88%] rounded-2xl px-4 py-3 text-sm leading-6 ${
                  message.role === 'user'
                    ? 'ml-auto bg-cyan-300/14 text-cyan-50'
            ...
            {isLoading && (
              <div className="max-w-[88%] rounded-2xl bg-slate-900 px-4 py-3 text-sm text-slate-400">
                Thinking...
              </div>
            )}
          </div>
      ...
      >
        <span className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-cyan-300/12">
